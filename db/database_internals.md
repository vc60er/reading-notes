...
1. 预准备(pre-prepare): 
	- 主节点广播pre-prepare消息，包含试图id，单调递增标识符，载荷（客户端请求），载荷的摘要（发送方签名）。如果，备份节点视图id与主节点的匹配。且请求未被篡改，那么备份节点会接受这条消息。
2. 准备(prepare): 
	+ 如果备份节点接受了预准备消息，它将进入准备阶段，并开始向其他所有副本(包括主副本)广播Prepare消息，其中包含视图ID、消息ID和载荷的摘要，但不包含载荷本身。只有当接收到 2f个来自不同备份节点的 Prepare 消息并且这些消息的视图、ID 和摘要均相同时，节点才能进入下个阶段。	
3. 提交(commit): 
 	- 之后，备份节点进入提交阶段，将 Commit消息广播到其他所有副本，并等待收集到2f+1条匹配的Commit消息(可能包括自己)
4. 客户端确认：
	- 客户端等待f+1个副本回复相同的结果，才确认执行成功	
