...
ZAB协议中有领导者和跟随者两种角色，其中领导者负责驱动整个算法流程，广播消息给跟随者并建立事件顺序。数据写入和最新数据的读取请求有领导者来处理。通过任意选举算法选举产生潜在领导者，然后再通过投票方式成为领导者。

ZAB协议中时间线被分割成了epoch，具有唯一且单调递增的序号标识，每个epoch中仅会存在一个领导者。

选举产生的潜在领导者，通过以下步骤成为真的领导者，并履行相关职责：

* 发现阶段：完成新epoch的提议和确认。
	- 潜在领导了解其他进程的epoch，并提出一个更大的epoch提议给跟随者，
	- 跟随者收到提议之后，回复在前一个epoch中看到过的最新事务标识符。
	- 之后，跟随者不再接受更早epoch的广播提议
* 同步阶段：完成新领导者的提议和确认，以及数据同步。
	- 潜在领导者提议自己是新epoch的领导，收到跟随者的确认之后，确立成为了领导者，
	- 之后跟随者不再接受其他潜在领导者成为领导者的提议。
	- 新领导者确保各个跟随者拥有相同的历史记录，并将先前epoch领导者已提交的提案转发给跟随者
* 广播阶段：
	- 领导者接收客户端的消息，确定消息顺序，并提议给跟随者，
	- 等待quorum的跟随者确认，最后提交。
...
- 消息有序性，通过消息确认来保障，在上一个消息确认之前，领导者不会尝试发送下一个消息
- 领导者选举，领导者未从quorum的跟随者那里收到心跳，会发起领导者选举。跟随者未存领导者收到心跳，也会发现领导者选举
...
1. 提议者发送一条Prepare(n)的消息（n是提议编号）给接受者
2. 接受者收到Prepare(n)消息：
	- 如果，n大于接受者曾经收到过的提议编号，则发送Promise(n,v[n])给提议者，并承诺不再接受其他编号更低的提案
	- 如果，接受者已经接受了（收到了Accept!(m，v[m])消息），则回复一条Promise(m,v[m])消息，通知提议者已经接受了编号为m的提案
	- 如果，接受者已经Promise了更高编号的请求，则回复Promise(m v[m])，通过提议者存在更高的提案
	- 接受者可以接收多个Prepare消息，并回复Promise，只要提议编号有序递增即可
3. 提议收到多数接受者的Promise消息之后，开始复制过程，提议者向接受者发送Accept!(n,v[max_from_accptor])来提交这个提案，v是从接受者收到的Promise回复中具有最高编号的提案的值。
4. 接受者收到Accepte!(n,v)之后，通知每个学习者，
5. 学习者收到多数接受者的通知之后，决定更新v


**总结：**

- 投票阶段：多个提议者竞争领导权(最终同步谁的提案的值)
- 复制阶段：提议者将值分发给接受者
...
- 提议者P1完成提案编号为1的选举阶段，但是他在发送值v1给接受者A1之后宕机
- 另一个提议者P2发起一个提案编号为2的新回合，收集Quorum个接受者的回复（本例中A1和A2），并用P1提出的旧值V1继续执行
...
- 提议者P1完成提案编号为1的选举阶段，但是他在发送值v1给接受者A1之后宕机
- 另一个提议者P2发起一个提案编号为2的新回合，收集Quorum个接受者的回复（本例中A2和A3），P2提交了v2

![pic](./pic/14-5.png)

提议者故障之后，接受者故障:

- 提议者P1完成提案编号为1的选举阶段，但是他在发送值v1给接受者A1之后宕机，A1接受之后也宕机
- 提议者P2发起新回合，它与A1的存活时间没重叠，继续执行并提交了V2
...
经典Paxos中，每次写入都需要进行一轮提议，然后获得多数投票之后，在进行复制。Multi-Paxos中，避免了重复进行提议阶段，提议者可以重用已被认可的地位，引入了领导者概念，显著提高了算法效率

经典Paxos中，每次读取也需要通过一轮Paxos回合实现。Multi-Paxos中，可以从已知的领导者那里读取。

有些Multi-Paxos实现了租约机制，领导者定期与参与者通信，通知它们自己还活着，同时延长租约
...
各个参与者在本地保存一份日志，记录了状态机执行命令的顺序，各进程接收的输入相同，日志中记录的命令及其顺序也是相同的。因此，将这些命令应用到状态机上一定会得到相同的输出。

**参与者角色：**

- 候选者：参与领导者选举
- 领导者：负责处理客户端的请求，维护复制的状态机。
- 跟随者：被动参与者，负责保存日志条目，以及响应领导者和候选者的请求，

领导者任期：领导者当选的时间段称为任期，每个任期用一个单调递增的数字标识，可以持续任意长的时间。
命令标识：任期编号和任期内消息标号标识
...
1. 参与者首先转变状态称为候选者
2. 候选者发起投票RequestVote(候选者任期，他知道的最后任期，最后日志ID)
3. 跟随者收到投票请求，如果投票的信息小于本地信息（最后任期，最后日志），则拒绝，否则投票，每个跟随者最多投票给一位候选者
4. 候选者如果收到了多数投票，则成为领导者，
5. 领导者定期向所有跟随者发送心跳以维持其任期
...
1. 领导者接受客户端的请求，向本地日志中追加新的条目，通过AppendEntries消息并行的复制给跟随者。AppendEntries该消息包含领导者的任期，前一条日志的下标和任期，以及一个或者多个要保存的日制条目
2. 跟随者没收到AppendEntries，将消息中的条目追加到本地日志中，并对此消息进行确认，告知领导者消息已被持久化。
3. 领导者收到多数跟随者的确认之后，该条目即被视为已提交。
...
1. 平票：领导者选举过程，候选者获得相同的票数。解决办法是，让随机让某个候选者先发起选举
2. 跟随者故障：领导者的同步消息未收到确认时，领导者可以再次发送同步消息
3. 新任期内的首次同步：新当选领导者先找到共同的基础点，然后通知跟随者丢去此后的所有未提交的日志。然后将自己日志中的最近条目发送给跟随者，覆盖其历史记录。

### 拜占庭共识

拜占庭问题是指，通信的参与者会存在恶意行为(如消息篡改或伪造)，除了人为的恶意外，拜占庭故障还可能由bug、错误配置、硬件问题或者数据损坏导致，

#### PBFT算法：

将定节点故障是独立的（故障不会相互配合，不能立即或者至少不会以相同的方式接管整个系统），节点间的所有通信经过加密，副本知道彼此的公钥，通过它来验证身份并加密消息。

如果，节点总数是n，则，最大容错节点数是f=(n-1)/3 

**角色：**

- 主节点：处理客户端请求，将请求广播给备份节点
- 备份节点：执行请求，并将响应发回客户端。

**算法过程：**

1. 预准备(pre-prepare): 
	- 主节点广播pre-prepare消息，包含视图id，单调递增标识符，载荷（客户端请求），载荷的摘要（发送方签名）。
	- 备份节点收到pre-prepare之后，如果，本地视图id与主节点视图id匹配，且请求未被篡改，那么备份节点会接受这条消息，则进入下一阶段
2. 准备(prepare): 
	- 完成预准备的备份节点向其他所有副本(包括主副本)广播Prepare消息，其中包含视图ID、消息ID和载荷的摘要，但不包含载荷本身。
	- 各节点等待并接收Prepare消息，如果，接收到2f个来自不同备份节点的Prepare消息，并且这些消息的视图ID和摘要均相同，则进入下一阶段
3. 提交(commit): 
 	- 完成准备阶段的节点将Commit消息广播到其他所有副本
 	- 各节点等待并接收Commit消息，如果，收集到2f+1条匹配的Commit消息(可能包括自己)，则进入下一阶段
4. 响应(response)：
	- 完成提交阶段的节点将结果回复给客户端，
	- 客户端等待并接收到f+1个副本回复相同的结果，才确认执行成功	
